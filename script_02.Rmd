...
library(readxl)
raw <- read_excel("~/Desktop/gov50/raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx") %>%
  slice(3:2786)
  colnames(raw) <- as.character(raw[1,]) 
  raw = raw[-1] %>%
  slice(2:2784) %>%
    select("Course Name", "UGrad" ) %>%
...
#The class with the most undergraduate enrollment is Ec10 
```
